...
(function() {
    ...
    $(".catalog tbody tr").each((i,tr)=>{
       ...
       const qq = $("input[name=q]").val().toLowerCase().split(" ").map(w=>w.trim());
       const ww=$(tr).find("td:eq(0)").text().toLowerCase().split(/[ .;]/).map(w=>w.trim())
...
const article =
    `${author} [${year}]. ${title2}. ${journal.trim()}, pp. `;

    console.log(article);

        $(tr).find("td:eq(1)")
            .append($(`<button>Copy citation</button>`).on("click",function(){
